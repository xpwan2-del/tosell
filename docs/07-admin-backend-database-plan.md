...
4. VirtualInventory：平台和商户 scoped 卡密导入、库存扣减、权益发放、商品级提取码配置、独立提取页 token/订单查询凭证、提取锁定、邮箱发码记录、退款后禁看。
...
4. 提取卡密：通过安全 token、订单查询凭证或登录态派生订单上下文，只校验购买密码/提取码、锁定状态、退款状态和订单归属；错误次数服务端累计。
