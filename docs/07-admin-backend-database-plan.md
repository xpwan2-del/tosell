...
8. 收款配置审核：支付宝商户、腾讯/微信商户、e支付、个人支付宝。
...
2. 收款配置：提交二维码/链接、查看审核状态、设置默认通道。
3. 商品选品：查看自己可见商品和自己可见上游供货价。
4. 商品上架：设置最终售价；商户自有自动卡密商品可通过商户 scoped 权限导入或查看自己的库存卡密，不能查看平台商品、上级商品或其他商户商品的卡密明文。
...
4. VirtualInventory：平台和商户 scoped 卡密导入、库存扣减、权益发放、商品级购买密码配置、独立提取页 token/订单查询凭证、提取锁定、邮箱发码记录、退款后禁看。
...
8. CollectionChannel：收款配置提交、审核、启用、禁用、订单通道快照。
...
1. 创建订单：校验店铺、销售商户保证金确认状态、商户、商品、库存、价格、优惠券、收款配置和风控；销售商户 `deposit_status != paid` 时后端硬拒绝创建可支付订单；通过后写订单、快照、支付记录。
2. 确认收款：锁定订单和支付记录；校验金额；写账务流水；触发履约；幂等处理。
3. 自动发码：锁定可用卡密；分配给订单；写权益和履约记录；重复请求不得重复发码。
4. 提取卡密：通过安全 token、订单查询凭证或登录态派生订单上下文，只校验购买密码、锁定状态、退款状态和订单归属；错误次数服务端累计。
