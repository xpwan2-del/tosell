...
1. e支付、支付宝、微信商户通道按后台通道配置收取支付手续费；当前 e支付按 `fee_bps=100` 展示和计算，即 1%。
2. 个人支付宝、个人微信和余额支付默认 `fee_bps=0`；支付手续费能力保留在后台配置中，运营可按通道调整比例。
3. 后端计算，前端只展示后端返回金额。
4. 订单和支付快照保存 `fee_bps`、`fee_cents`、`base_amount_cents`、`payable_amount_cents`。
5. 支付手续费按人民币分向上取整；通道手续费比例大于 0 且订单金额大于 0 时，最小手续费为 1 分。
...
订单页必须把购买密码和联系电话明确标注为“必填”。购买密码输入框必须提示至少输入 4 位。
